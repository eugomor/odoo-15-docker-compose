...
``` bash
curl -s https://raw.githubusercontent.com/eugomor/odoo-15-docker-compose/feature_https_support/run.sh | sudo bash -s odoo-one 10015 20015
```

to set up first Odoo instance @ `localhost:10015` (default master password: `eugomor.info`)

and

``` bash
curl -s https://raw.githubusercontent.com/eugomor/odoo-15-docker-compose/master/run.sh | sudo bash -s odoo-two 11015 21015
```

to set up another Odoo instance @ `localhost:11015` (default master password: `eugomor.info`)
...
``` sh
$ git clone https://github.com/eugomor/odoo-15-docker-compose
...
* Default database password (**admin_passwd**) is `eugomor.info`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
